...
$('#modalDelete').on('show.bs.modal', function (event) {
  ...
  $('#modalApprove').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget)
    var approve_id = button.data('approve-id')
    var repairname = button.data('repairname')
    var modal = $(this)
    modal.find('.modal-body span').text(alert_approve_modal)

    $("#modalApprove .btn-continue").off();
    $("#modalApprove .btn-continue").on("click", function(){
        window.location.href="apps/usecar/do_usecar.php?action=endjob&id="+approve_id;
    })
  })
